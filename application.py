...
def buy():
    ...
    if request.method == "GET":
        ...
        # get info from submit form
        stock = lookup(request.form.get("symbol"))
        no_shares = int(request.form.get("shares"))
        if not stock:
            return apology("Invalid stock symbol")
        total_amount = stock["price"] * no_shares

        #db query to see if you have enough money
        rows = db.execute("SELECT * FROM users WHERE id = :id", id = session["user_id"])
        if rows[0]["cash"] < total_amount:
            return apology("Insufficient funds")

        else:
            # change cash value in user db
            db.execute("UPDATE users SET cash = ? WHERE id = ?", (rows[0]["cash"] - total_amount, session["user_id"]))

            # insert transaction into stocks db
            db.execute("INSERT INTO trades (user_id, symbol, price, shares) VALUES (?, ?, ?, ?)",
                        (session["user_id"], stock["symbol"], stock["price"], no_shares))
            return redirect("/")
